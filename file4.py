...
def downf4():
    ...
    time.sleep(2)
    user_input = driver.find_element_by_id('ctl00_ctl00_ctl00_placeHolderMain_mainWebCad_FilterControls_frmFiltro_dcpDataNecessidadeInicialCtrl50_dateInput')
    user_input.send_keys(functions.data_primeiro_dia)
    time.sleep(3)
    user_input = driver.find_element_by_id('ctl00_ctl00_ctl00_placeHolderMain_mainWebCad_FilterControls_frmFiltro_dcpDataNecessidadeFinalCtrl51_dateInput')
...
def rename_file4():
    ...
    new_nn = 'CM.CMFlex.Relatorios.Reports.ConfiguracaoDeRelatorioReport.mrt_REQ_' + ("{:04d}".format(functions.ano_atual)) + ("{:02d}".format(functions.mes_atual)) + '.xlsx'
